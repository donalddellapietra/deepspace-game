...
fn pack_voxel(voxel: LeafVoxel) -> u32 {
    // The game only reads palette_index out of each voxel — normal,
    // metallic, roughness, emissive were used by the upstream
    // raymarcher but discarded on our `.vxs` readback path. The high
    // bit is a presence flag so the CPU scan can distinguish "wrote
    // palette 0 here" from "uninitialized chunk texel".
    //
    // Layout: bit 31 = 1 (presence), bits 0-30 = palette_index.
    return (1u << 31u) | voxel.palette_index;
}
