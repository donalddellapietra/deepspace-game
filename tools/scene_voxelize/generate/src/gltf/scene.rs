...
impl Scene {
    pub fn from_gltf(gltf: &schema::Gltf) -> Result<Self> {
        ...
        // Upstream (voxel-raymarching) converts glTF Y-up to Z-up for
        // its renderer. Our game is Y-up natively, same as glTF — so
        // we seed the BFS with IDENTITY. Without this fix Sponza came
        // out rotated 90° around X (columns appearing as horizontal
        // spokes radiating from the center).
        for node in &scene.nodes {
            visit_queue.push_back((*node, glam::Mat4::IDENTITY));
        }
