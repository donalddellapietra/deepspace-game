...
//!
//! Caveat — boundary crossings near face TBs are *not* well-defined.
//! Because the slot direction `(sx − 1, sy − 1, sz − 1)` and the TB
//! rotation's `+Y` image disagree by ~32°, the world-axis-aligned slot
//! extent and the rotated storage-frame extent don't coincide. The
//! `from_world_xyz` ↔ `in_frame_rot` pair is still an exact inverse
//! for any world point (and the round-trip tests below cover all
//! twelve faces), but the boundary-crossing renormaliser used by
//! `add_local` cannot pick a unique owner for points in the overlap
//! region. This is intrinsic to placing non-axis-aligned rotations at
//! a coarse grid slot, not a bug in this preset. Movement around the
//! figure works fine *within* a face TB and within the centre.
...
pub const DODECAHEDRON_TEST_TREE_DEPTH: u32 = CUBE_SUBTREE_DEPTH as u32 + 1;
...
pub fn dodecahedron_test_world() -> WorldState {
...
pub fn dodecahedron_test_spawn() -> WorldPos {
...
mod tests {
    ...
    fn tree_has_expected_depth() {
        let world = dodecahedron_test_world();
        assert_eq!(world.tree_depth(), DODECAHEDRON_TEST_TREE_DEPTH);
    }
    ...
    /// Round-trip every face: a world point inside each face-TB cube
    /// must come back unchanged through `from_world_xyz` →
    /// `in_frame_rot`. Proves that the storage-frame slot derivation
    /// (R^T at TB descend) and the world-position derivation (R at
    /// `in_frame_rot` accumulation) are exact inverses for every
    /// non-axis-aligned dodecahedron rotation.
    ///
    /// Note: this preset uses approximate (rounded-to-grid) slot
    /// positions for the twelve face TBs, so the world-axis-aligned
    /// slot extent and the rotated storage-frame extent do not coincide.
    /// `from_world_xyz` / `in_frame_rot` are still exact inverses, but
    /// boundary-crossing renormalisation (`add_local` etc.) is *not*
    /// well-defined near a face-TB boundary because the renormaliser
    /// can't decide which TB owns a world point that lands in the
    /// overlap region. That's a property of any non-axis-aligned
    /// rotation placed at a grid slot — not a bug in this preset, and
    /// not exercised by these inverse-pair tests.
    #[test]
    fn from_world_xyz_round_trips_through_every_face() {
        use crate::world::anchor::Path;
        let world = dodecahedron_test_world();
        // For each face slot pick a non-symmetric world point inside
        // the slot. Slot extents in root frame coords are
        // [sx, sx+1) × [sy, sy+1) × [sz, sz+1); we pick (sx+0.62,
        // sy+0.43, sz+0.71) — irrational fractions break any planar
        // symmetry that would mask a rotation-component swap.
        for (sx, sy, sz, _) in face_table() {
            let xyz = [
                sx as f32 + 0.62,
                sy as f32 + 0.43,
            ...
            for i in 0..3 {
                assert!(
                    (recovered[i] - xyz[i]).abs() < 1e-4,
                    "round-trip drift at face slot ({sx},{sy},{sz}) axis {i}: \
