...
///
/// Each preserve region is `(path, extra_depth)`, meaning:
/// do not uniform-collapse or distance-LOD flatten any Cartesian child whose
/// path stays under `path` and whose depth is at most `path.depth()+extra_depth`.
///
/// This is the missing piece for truly local Cartesian render roots: preserving
/// only the exact ancestor chain is not enough, because the near field around a
/// local render frame still needs a bounded neighborhood of detailed children.
pub fn pack_tree_lod_selective(
    ...
    fn in_preserve_region(path: &Path, preserve_regions: &[(Path, u8)]) -> bool {
        preserve_regions.iter().any(|(root, extra_depth)| {
            let required_depth = root.depth().saturating_add(*extra_depth);
            path.depth() <= required_depth && path.common_prefix_len(root) == root.depth()
        })
    }
