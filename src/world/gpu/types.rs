...
/// Largest uniform scale factor `s` such that a `[0, 3s)³` cube
/// centred at `(1.5, 1.5, 1.5)`, after rotation `R`, fits entirely
/// within `[0, 3)³`. For each axis `i`, the rotated cube's
/// half-extent along `i` is `1.5 · s · Σ_j |R[j][i]|`. Setting that
/// `≤ 1.5` gives `s ≤ 1 / max_i(Σ_j |R[j][i]|)`. For a 45° Y rotation
/// this is `1/√2 ≈ 0.707`.
...
mod tests {
    ...
    fn from_node_kind_tangent_block_carries_rotation() {
        ...
        assert_eq!(k.kind, 2);
        // Identity rotation has inscribed scale 1.0 (no shrink needed).
