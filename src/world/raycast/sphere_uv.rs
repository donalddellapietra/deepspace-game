...
/// `[0, 2)³` coords. The function ray-intersects the implied
...
pub fn cpu_raycast_sphere_uv(
    ...
    // Sphere center / radius in the slab-root frame's [0, 2)³ space.
    let cs_center = [1.0_f32, 1.0, 1.0];
    ...
    for cy in (0..dims[1] as i32).rev() {
        ...
        // multiplies the resolution by 2, so the break path lands at
        ...
        if let Some(mut node_idx) = sub_idx {
            ...
            if extra_levels > 0 {
                ...
                // outermost r-slot (sy = 1), which is the cell at
                ...
                if is_proto {
                    if let Some((cx_2, cy_2, cz_2)) = cube_local_hit(
                        cam_local, dir,
                        cs_center, lat_max,
                        cell_x, cy, cell_z,
                        dims, shell_thickness, r_inner,
                    ) {
                        // cx_2 / cy_2 / cz_2 are in cube-local
                        // [0, 2)³. Convert to [0, 1) fractions for
                        // the descent loop below.
                        frac_x = (cx_2 / 2.0).clamp(0.0, 0.99999);
                        frac_y = (cy_2 / 2.0).clamp(0.0, 0.99999);
                        frac_z = (cz_2 / 2.0).clamp(0.0, 0.99999);
                    }
                }
                for _ in 0..extra_levels {
                    let sx = (frac_x * 2.0).floor() as usize;
                    let sy = (frac_y * 2.0).floor() as usize;
                    let sz = (frac_z * 2.0).floor() as usize;
                    ...
                    frac_x = (frac_x * 2.0) - sx as f32;
                    frac_y = (frac_y * 2.0) - sy as f32;
                    frac_z = (frac_z * 2.0) - sz as f32;
                }
            }
        }
...
/// Returns the hit position in cube-local [0, 2)³ coords on hit,
/// or `None` if the ray misses the cube.
///
/// CPU mirror of the GPU cube basis:
///   cube.x = lon-tangent
///   cube.y = +radial out  (cube.y=0 inner, cube.y=2 outer)
///   cube.z = lat-tangent
///
/// For ray hits, the (cx_2, cy_2, cz_2) returned is the entry
/// position in cube-local coords. Caller divides by 2 to get
...
fn cube_local_hit(
    ...
    // Hit position in cube-local, mapped to [0, 2)³.
    let hit_l = [
        pos_l[0] + dir_l[0] * t_hit,
        pos_l[1] + dir_l[1] * t_hit,
        pos_l[2] + dir_l[2] * t_hit,
    ];
    let cx_2 = (hit_l[0] + half_u) * 2.0 / (2.0 * half_u);
    let cy_2 = (hit_l[1] + half_v) * 2.0 / (2.0 * half_v);
    let cz_2 = (hit_l[2] + half_w) * 2.0 / (2.0 * half_w);
    Some((
        cx_2.clamp(0.0, 1.99999),
        cy_2.clamp(0.0, 1.99999),
        cz_2.clamp(0.0, 1.99999),
    ))
}
...
mod tests {
    ...
    fn cpu_raycast_sphere_uv_east_equator_hits_middle_cell() {
        ...
        // Camera way east of sphere center (1.0, 1.0, 1.0), looking
        // -X. Ray hits the +X equator point at world-frame
        // (1.0 + R, 1.0, 1.0).
        let cam_local = [2.0, 1.0, 1.0];
        ...
        // cell_x = floor(0.5 * 8) = 4, cell_z = floor(0.5 * 4) = 2,
        // cell_y = dims_y - 1 = 1 (GRASS row).
        // The path's leaf-level (NodeId, slot) encodes the cell —
        // leaf-level slot for (cell_x % 2, cell_y % 2, cell_z % 2) =
        // (4 % 2, 1 % 2, 2 % 2) = (0, 1, 0) = slot 2.
        let leaf_slot = hit.path.last().expect("hit path non-empty").1;
        assert_eq!(leaf_slot, slot_index(0, 1, 0),
            "expected leaf slot (0,1,0) for cell (4, 1, 2), got slot {leaf_slot}");
    ...
    fn cpu_raycast_sphere_uv_dug_grass_reveals_layer_below() {
        ...
        // Build a slab with a HOLE at the +X equator cell.
        // With [8, 2, 4] and slab_depth=3: cell_x=4, cell_y=1, cell_z=2.
        let mut world = wrapped_planet_world(2, [8, 2, 4], 3, 1);
        ...
        // down to (cell_x=4, cell_y=1, cell_z=2) at slab_depth=3.
        // Compute slot indices at each level (cells_per_slot 4, 2, 1).
        let path_slots = {
            let cx = 4i32; let cy = 1i32; let cz = 2i32;
            let mut slots = Vec::new();
            let mut cps = 4i32;
            for _ in 0..3 {
                let sx = (cx / cps).rem_euclid(2) as usize;
                let sy = (cy / cps).rem_euclid(2) as usize;
                let sz = (cz / cps).rem_euclid(2) as usize;
        ...
        // cell (4, 0, 2) is (0, 0, 0) = slot_index(0, 0, 0).
        let frame_path = vec![slot_index(1, 1, 1) as u8; 2];
        let cam_local = [2.0, 1.0, 1.0];
        let ray_dir = [-1.0, 0.0, 0.0];
        let hit = cpu_raycast_sphere_uv(
        ...
        // Expect cell_y = 0 → leaf slot has sy = 0 → slot_index(0, 0, 0) = 0.
        assert_eq!(leaf_slot, slot_index(0, 0, 0),
            "expected leaf slot (0,0,0)=0 (cell_y=0 = stone) after digging grass, got {leaf_slot}");
    }
    ...
    /// original block type, because 1/8 cells empty leaves the
    ...
    fn diagnosis_sphere_deep_break_invisible_at_slab_granularity() {
        ...
        let case = |max_depth: u32| {
            let mut world = wrapped_planet_world(2, [8, 2, 4], 3, 3);
            let frame_path = vec![slot_index(1, 1, 1) as u8; 2];
            let cam_local = [2.0, 1.0, 1.0];
            let ray_dir = [-1.0, 0.0, 0.0];
            let hit = cpu_raycast_sphere_uv(
        ...
        // representative_block is still GRASS (1 of 8 sub-cells empty
        ...
        // Same pattern at edit-depth=7 and 8: each break is 1/8^k of
