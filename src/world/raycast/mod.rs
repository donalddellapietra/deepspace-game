...
/// Re-export of the inscribed-cube scale (defined in `tree`). The
/// CPU raycast applies the same shrink at the same transform points
/// the GPU shader does, so cursor targeting matches the visual.
pub(crate) use crate::world::tree::inscribed_cube_scale;
