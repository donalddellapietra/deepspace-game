...
//! in that frame's coordinates. Both Cartesian and body-root
//! UV-sphere frames are linear `[0, 3)³` — the UV sphere body's
//! children are addressed by the same `node.children[slot]`
//! convention as cartesian, just with `slot` interpreted as
//! `(pt, tt, rt)` UV tiers shader-side.
//!
...
pub enum ActiveFrameKind {
    Cartesian,
    /// Render frame is a `NodeKind::UvSphereBody` node. The shader
    /// dispatches the UV-sphere DDA at depth 0; body params are
    /// read shader-side from `node_kinds[uniforms.root_index]`.
    UvSphereBody {
        inner_r: f32,
        outer_r: f32,
        theta_cap: f32,
...
/// `desired_depth` slot steps. Stops at any `UvSphereBody` node —
/// the shader dispatches the matching DDA at the body root. The
/// body's children carry on inside the shader's `march_uv_sphere`
/// without further frame descent CPU-side.
