...
/// the bent ray paths shift and prev frame's hits don't reproject to
/// the current frame's pixels under the new bend, so we invalidate
/// history. Within a bucket k drift is small enough that TAA's
/// neighborhood clamp absorbs it.
