...
pub struct GpuUniforms {
    ...
    pub curvature: [f32; 4],
    /// `.y = lat_max` (radians) — pole-band ban threshold for the
    /// `WrappedPlane` rotated-tangent-cube render. `.xzw` reserved.
...
pub struct Renderer {
    ...
    /// Mirror of `uniforms.planet_render`. `.y` = lat_max (radians).
...
impl Renderer {
    ...
    /// from a stale upload.
    pub fn set_frame_tb_center(&mut self, c: [f32; 3]) {
        self.frame_tb_center = [c[0], c[1], c[2], 0.0];
    }
    ...
    /// Polar-band ban threshold for `WrappedPlane` rendering, in
