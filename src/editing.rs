...
///
/// The cubed-sphere shell is a separate tree whose depth is capped
/// below `f32` integer precision: past depth 14 the shader's
/// `floor(un · 3^depth) == hl_i` comparison stops being exact.
/// We also floor at 1 so there's always something to edit. Within
/// that window, the cubed-sphere edit depth just mirrors the
/// anchor depth the player is operating at.
pub fn cs_edit_depth(cs_planet: Option<&SphericalPlanet>, anchor_depth: u32) -> u32 {
...
pub fn cs_cursor_hit(
    ...
    let depth = cs_edit_depth(cs_planet, edit_depth);
...
pub fn try_cs_break(
    ...
    let hit = {
        ...
        match cs_cursor_hit(world, planet_ref, camera_pos, ray_dir, edit_depth) {
            Some(h) => h,
            None => return false,
        }
    };
