...
impl TestWorld {
    fn from_library(
        ...
        // fractal-presets extended pack_tree to return the parallel
        // AABB buffer (6-tuple). Heightmap tests walk the tree
        // directly — they don't need the per-node AABB cull — so
        // we drop that slot here.
        let (tree, _node_kinds, node_offsets, _aabbs, _node_ids, root_bfs) =
            pack_tree(library, root);
