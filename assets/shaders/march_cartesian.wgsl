// Cartesian DDA — `march_cartesian` walks an axis-aligned `[0, 3)³`
// frame using a stack-based descent that matches the storage tree's
// 27-children Cartesian layout. Includes:
//
//   * `march_entity_subtree`: per-entity sub-DDA invoked from the
//     `tag=3` branch of `march_cartesian` when the cell holds an
//     entity overlay.
//   * `march_cartesian`: the main per-frame DDA. Handles X-wrap at
//     `depth==0` for `WrappedPlane` root frames (Phase 2), tag=1
//     leaves, tag=2 Node descent with content-AABB cull, tag=3
//     entity dispatch, and stack-based depth management capped at
//     `MAX_STACK_DEPTH`.
//
// Helpers (`pack_cell` / `unpack_cell` / `path_mask_conservative` /
// `normal_to_face` / `write_walker_probe`) live in `march.wgsl`
// because they're shared with the sphere DDAs.
