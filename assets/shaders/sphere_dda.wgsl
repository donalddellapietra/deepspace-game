...
// node. Operates in the WP's local `[0, 2)³` frame with the
// implied sphere centered at `(1.0, 1.0, 1.0)` of radius
...
//     anchor's 8-children grid, recursively descending until each
...
// * 8-children descent — each level divides (lon, lat, r) cell sizes
//   by 2.
...
fn sphere_descend_anchor(
    ...
    // Frame 0 = anchor block. Each axis is 1/2 of the slab cell.
    var cur_lon_org = slab_lon_lo;
    var cur_lat_org = slab_lat_lo;
    var cur_r_org   = slab_r_lo;
    var cur_lon_step = slab_lon_step / 2.0;
    var cur_lat_step = slab_lat_step / 2.0;
    var cur_r_step   = slab_r_step   / 2.0;
    ...
    // Initial sub-cell from the ray's t.
    {
        ...
        let cx0 = clamp(i32(floor((lon0 - cur_lon_org) / cur_lon_step)), 0, 1);
        let cy0 = clamp(i32(floor((r0   - cur_r_org)   / cur_r_step)),   0, 1);
        let cz0 = clamp(i32(floor((lat0 - cur_lat_org) / cur_lat_step)), 0, 1);
    ...
    loop {
        ...
        let cx_c = clamp(i32(floor((lon_c - cur_lon_org) / cur_lon_step)), 0, 1);
        let cy_c = clamp(i32(floor((r_c   - cur_r_org)   / cur_r_step)),   0, 1);
        let cz_c = clamp(i32(floor((lat_c - cur_lat_org) / cur_lat_step)), 0, 1);
...
// / (2π)`, body_size = 2 (= the WP's local frame size).
//
...
fn render_cell_as_tangent_cube(
    ...
    // (sy=0 inner, sy=1 outer), slot.z → lat. So:
...
// transformed into cell-local [0, 2)³ coords and
...
fn cartesian_voxels_in_cell(
    ...
    // Transform ray to cell-local [0, 2)³ frame. Each cell-local
    // unit corresponds to `2 * half / 2` world meters along each
    // OBB axis. Sharing the t parameter with world (P_2(t) =
    // pos_2 + t * dir_2 corresponds to P_world(t) = O_world +
    // t * ray_dir): pos_2 = (pos_l + half) * 2 / (2*half), and
    // dir_2 = dir_l * 2 / (2*half).
    let scale_to_2 = vec3<f32>(2.0) / (2.0 * half_v3);
    let pos_2 = (pos_l + half_v3) * scale_to_2;
    let dir_2 = dir_l * scale_to_2;

    let cart_hit = march_in_proto_cube(sub_node_idx, pos_2, dir_2);
    ...
    // cell-local [0, 2)³ coords (the frame we passed to
    // march_cartesian); compute UV inside the hit cell and
    // smoothstep near the edges.
    let entry_2 = pos_2 + dir_2 * cart_hit.t;
    let uvw_in_cell = (entry_2 - cart_hit.cell_min) / cart_hit.cell_size;
    ...
    loop {
        ...
        if sample.block_type != 0xFFFEu {
            ...
            if sample.tag == 2u {
                ...
                // in the cell's tangent-plane local [0, 2)³ frame
...
// `parent_pos = slot_xyz + frame_pos / 2`, `parent_dir = frame_dir / 2`.
