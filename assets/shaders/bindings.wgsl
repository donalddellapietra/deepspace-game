...
struct Camera {
    ...
    /// Cumulative rotation (unit quaternion x,y,z,w) of the render
    /// frame relative to world. Identity outside any TangentBlock;
    /// equal to the cumulative product of TangentBlock rotations
    /// along the camera anchor path otherwise. shade_pixel applies
    /// this to hit normals so world-frame lighting works on either
    /// side of a cube boundary.
...
struct NodeKindGpu {
    ...
    /// Slab dims (cells/axis) for WrappedPlane; zero otherwise.
    dims_x: u32,
    dims_y: u32,
    dims_z: u32,
    /// Unit quaternion (x, y, z, w) — the cube's orientation for
    /// `TangentBlock`. Identity (0, 0, 0, 1) for other kinds.
    /// `march_cartesian`'s TangentBlock dispatch reads this directly
    /// — single source of truth for the rotation.
    rotation: vec4<f32>,
}
