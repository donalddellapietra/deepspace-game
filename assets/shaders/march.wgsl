...
// Position-tracking cartesian DDA, precision-stable at deep depth.
//
// State: every level operates in its own [0, 3]³ local frame, where
// cells are 1 unit. The ray's CURRENT position in this frame is held
// in `cur_pos` and updated incrementally on each cell crossing — we
// never reconstruct the position via `origin + dir * t` (that'd
// subtract two ~3^N values to land in [0, 3], catastrophic at deep
// depth). `cur_dir` scales by 3 per push, by 1/3 per pop. Per-step
// position deltas are O(1):
//
//   side_dist = (next_face - cur_pos) * cur_inv_dir
//   min_t     = min(side_dist)
//   cur_pos  += min_t * cur_dir            // O(1) magnitude
//   cur_pos[crossed] = next_face[crossed]  // snap exactly
//   cur_pos[crossed] += step * face_eps    // nudge into next cell
//
// Cell selection is precision-exact (`floor(cur_pos)`) at every depth.
//
// `cur_t_world` accumulates `min_t` for `result.t` and the LOD ray-
// distance estimate. Past depth ~17 the per-cell `min_t` falls below
// ULP of the running `cur_t_world` so its precision degrades — but
// `cur_pos` and the cells visited stay correct, so what's drawn is
// what's there.
fn march_entity_subtree(
    ...
    // Direction sign and "next face" offset — invariant under push/pop.
    let step = vec3<i32>(
    ...
    // Initial ray-box test → advance cur_pos to the entry point.
    ...
    loop {
        ...
        // Out-of-bounds: ray exited the current frame's [0, 3]³.
        if cell.x < 0 || cell.x > 2 || cell.y < 0 || cell.y > 2 || cell.z < 0 || cell.z > 2 {
            if depth == 0u { break; }
            depth -= 1u;
            let popped = unpack_cell(s_cell[depth]);
            // Pop: child's frame → parent's frame. Linear, O(1).
        ...
        var should_step = (cur_occupancy & slot_bit) == 0u;
        ...
        if should_push {
            // Push: parent's frame → child's frame. cell is integer
            // in [0, 2], so `cur_pos - cell` is exact and `* 3` is
            // exact in f32; cur_pos lands in [0, 3]³ precisely.
        ...
        // Step DDA: advance to the next axis face in the current cell.
        ...
        // O(1) magnitude — exactly 1 on the crossed axis, ratios of
        // dirs on the other axes — so cur_pos stays well-conditioned.
        cur_pos = cur_pos + min_t * cur_dir;
        cur_t_world = cur_t_world + min_t;

        // Snap the crossed axis to the face value (kills f32 drift
        // around the boundary) and nudge into the next cell so the
        // next iteration's `floor(cur_pos)` lands cleanly there.
        if min_axis == 0u {
