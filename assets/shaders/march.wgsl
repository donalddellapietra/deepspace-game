...
fn march_cartesian(
    ...
    loop {
        ...
        if tag == 1u {
            ...
            // TangentBlock dispatch. Frame-local: re-express the ray in
            // the child's [0, 3)³, apply the stored rotation around the
            // cube center (1.5, 1.5, 1.5), dispatch the cartesian DDA
            // inside, transform normal back on hit. NO world-absolute
            // coordinates — every step stays in [0, 3)³ frame-local.
            if node_kinds[child_idx].kind == NODE_KIND_TANGENT_BLOCK {
                let child_origin_tb = cur_node_origin + vec3<f32>(cell) * cur_cell_size;
                let child_cell_size_tb = cur_cell_size / 3.0;
                // Step 1: ray into child's [0, 3)³ frame.
                let inv_csize = 1.0 / cur_cell_size;
                let local_origin_pre = (ray_origin - child_origin_tb) * inv_csize;
                let local_dir_pre = ray_dir * inv_csize;
                // Step 2: rotate around cube center (1.5, 1.5, 1.5).
                // Stored rotation R = (col0, col1, col2). Apply R^T:
                // (R^T · v).i = dot(coli, v).
                ...
                let centered = local_origin_pre - vec3<f32>(1.5);
                ...
                let local_dir = vec3<f32>(
                    dot(rc0, local_dir_pre),
                    dot(rc1, local_dir_pre),
                    dot(rc2, local_dir_pre),
                );
                let sub = march_in_tangent_cube(child_idx, local_origin, local_dir);
                if sub.hit {
                    // Bevel uses local-in-cell — already correct since
                    // sub.cell_min/cell_size are local. Recompute here
                    ...
                    // sub.t is parameter on (local_origin, local_dir)
                    // which equals world parameter (scale cancels).
                    var out: HitResult;
                    out.hit = true;
                    out.t = sub.t;
                    out.color = sub.color * (0.7 + 0.3 * local_bevel);
                    // Rotate normal back to outer frame: world = R · local.
                    // R has columns rc0/rc1/rc2 (col-major), so
...
fn march(world_ray_origin: vec3<f32>, world_ray_dir: vec3<f32>) -> HitResult {
    ...
    loop {
        ...
        // Single dispatch: cartesian DDA. NodeKind dispatch (TangentBlock
        // rotation transform) happens INSIDE march_cartesian at child
        // descent — no per-frame branch here.
