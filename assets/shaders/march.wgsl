...
// Rotated-45°-about-Y subtree walker.
//
// Triggered by `march_cartesian`'s `kind == 3u` dispatch when the
// outer DDA enters a cell whose child node is `NodeKind::Rotated45Y`.
// The subtree inside is an ordinary cartesian base-3 tree; only the
// rendering frame is rotated. Geometrically, the rotated block is
// inscribed in the parent cell's AABB — 4 top-down vertices touch
// the cell-edge midpoints in XZ, Y fills the full cell. This means
// internal cells are stretched cuboids in world space (1×√2×1
// aspect) while staying unit cubes in the rotated local frame.
//
// Ray transform — linear, applied relative to the cell center:
//     T(x, y, z) = (x - z, y, x + z)
// which is R_y(-45°) composed with a √2 XZ stretch. T maps the
// inscribed prism {|x|, |z| ≤ cs/(2√2)} × {|y| ≤ cs/2} to the cube
// [-cs/2, cs/2]³, so the inner DDA runs in a standard [0, 3)³ local
// frame. T⁻¹(u, y, w) = ((u+w)/2, y, (w-u)/2).
//
// On hit the t parameter is recovered in world coordinates via
// ray-direction projection (T isn't a pure scale, so simple t/scale
// doesn't apply). The face normal is rotated back via (T⁻¹)ᵀ —
// which, once normalized, turns local axis-aligned normals into
// the world-space 45°-diagonal normals expected for a diamond's
// side faces, while local Y normals stay vertical.
//
// Entity cells (tag==3) are not supported inside a rotated subtree
// — treated as miss. Sphere / rotated nested kinds likewise not
// dispatched here (kind==0 Cartesian descents only).
fn march_rotated45y_subtree(
    root_node_idx: u32,
    cell_min: vec3<f32>,
    cell_size: f32,
    ray_origin: vec3<f32>,
    ...
    result.frame_level = 0u;
    result.frame_scale = 1.0;
    // Coarse anchor: the outer cartesian cell's AABB. cell_min /
    // cell_size from the caller — precise enough for ribbon-pop
    // re-anchoring because the rotated cell's world footprint is
    // axis-aligned (it's the parent cell).
    result.cell_min = cell_min;
    result.cell_size = cell_size;

    let cell_center = cell_min + vec3<f32>(cell_size * 0.5);
    let pc = ray_origin - cell_center;
    // T(p) = (p.x - p.z, p.y, p.x + p.z) — 45° Y rotation + √2 XZ
    // stretch, folded into a single 6-add linear map.
    let pc_t = vec3<f32>(pc.x - pc.z, pc.y, pc.x + pc.z);
    let dc_t = vec3<f32>(ray_dir.x - ray_dir.z, ray_dir.y, ray_dir.x + ray_dir.z);
    let s3 = 3.0 / cell_size;
    let local_origin = (pc_t + vec3<f32>(cell_size * 0.5)) * s3;
    let local_dir = dc_t * s3;
    ...
    let ray_metric = max(length(local_dir), 1e-6);
    let step = vec3<i32>(
        select(-1, 1, local_dir.x >= 0.0),
        select(-1, 1, local_dir.y >= 0.0),
        select(-1, 1, local_dir.z >= 0.0),
    );
    let delta_dist = abs(inv_dir);
    ...
    let root_hit = ray_box(local_origin, inv_dir, vec3<f32>(0.0), vec3<f32>(3.0));
    if root_hit.t_enter >= root_hit.t_exit || root_hit.t_exit < 0.0 {
        return result;
    }
    let t_start = max(root_hit.t_enter, 0.0) + 0.001;
    let entry_pos = local_origin + local_dir * t_start;
    let entry_cell = vec3<i32>(
        clamp(i32(floor(entry_pos.x)), 0, 2),
        clamp(i32(floor(entry_pos.y)), 0, 2),
        clamp(i32(floor(entry_pos.z)), 0, 2),
    ...
    cur_side_dist = vec3<f32>(
        select((cell_f.x - entry_pos.x) * inv_dir.x,
               (cell_f.x + 1.0 - entry_pos.x) * inv_dir.x, local_dir.x >= 0.0),
        select((cell_f.y - entry_pos.y) * inv_dir.y,
               (cell_f.y + 1.0 - entry_pos.y) * inv_dir.y, local_dir.y >= 0.0),
        select((cell_f.z - entry_pos.z) * inv_dir.z,
               (cell_f.z + 1.0 - entry_pos.z) * inv_dir.z, local_dir.z >= 0.0),
    );

    let dd = max(dot(ray_dir, ray_dir), 1e-12);
    ...
    loop {
        ...
        if cell.x < 0 || cell.x > 2 || cell.y < 0 || cell.y > 2 || cell.z < 0 || cell.z > 2 {
            ...
            cur_side_dist = vec3<f32>(
                select((cur_node_origin.x + lc_pop.x * cur_cell_size - entry_pos.x) * inv_dir.x,
                       (cur_node_origin.x + (lc_pop.x + 1.0) * cur_cell_size - entry_pos.x) * inv_dir.x, local_dir.x >= 0.0),
                select((cur_node_origin.y + lc_pop.y * cur_cell_size - entry_pos.y) * inv_dir.y,
                       (cur_node_origin.y + (lc_pop.y + 1.0) * cur_cell_size - entry_pos.y) * inv_dir.y, local_dir.y >= 0.0),
                select((cur_node_origin.z + lc_pop.z * cur_cell_size - entry_pos.z) * inv_dir.z,
                       (cur_node_origin.z + (lc_pop.z + 1.0) * cur_cell_size - entry_pos.z) * inv_dir.z, local_dir.z >= 0.0),
            );
            let parent_header_off = node_offsets[s_node_idx[depth]];
            cur_occupancy = tree[parent_header_off];
            cur_first_child = tree[parent_header_off + 1u];
        ...
        // tag == 2u: Cartesian Node descent. tag==3 (EntityRef) is
        // not supported inside rotated subtrees — treat as miss.
        if tag != 2u {
            let m_skip = min_axis_mask(cur_side_dist);
            s_cell[depth] = pack_cell(cell + vec3<i32>(m_skip) * step);
            cur_side_dist += m_skip * delta_dist * cur_cell_size;
        ...
        if at_max || at_lod {
            ...
            if bt == 0xFFFEu || bt == 0xFFFDu {
                ...
                let cell_min_l2 = cur_node_origin + vec3<f32>(cell) * cur_cell_size;
                let cell_max_l2 = cell_min_l2 + vec3<f32>(cur_cell_size);
                ...
                let p_world_off2 = vec3<f32>(
                    ...
                    p_shifted2.y,
                    (p_shifted2.z - p_shifted2.x) * 0.5,
                );
                let p_world2 = p_world_off2 + cell_center;
            ...
            let child_origin = cur_node_origin + vec3<f32>(cell) * cur_cell_size;
            let ct_start = max(root_hit.t_enter, 0.0) + 0.0001 * child_cell_size;
            let child_entry = local_origin + local_dir * ct_start;
            let local_entry = (child_entry - child_origin) / child_cell_size;
            depth += 1u;
            s_node_idx[depth] = child_idx;
            cur_node_origin = child_origin;
            ...
            cur_side_dist = vec3<f32>(
                select((child_origin.x + lc.x * child_cell_size - entry_pos.x) * inv_dir.x,
                       (child_origin.x + (lc.x + 1.0) * child_cell_size - entry_pos.x) * inv_dir.x, local_dir.x >= 0.0),
                select((child_origin.y + lc.y * child_cell_size - entry_pos.y) * inv_dir.y,
                       (child_origin.y + (lc.y + 1.0) * child_cell_size - entry_pos.y) * inv_dir.y, local_dir.y >= 0.0),
                select((child_origin.z + lc.z * child_cell_size - entry_pos.z) * inv_dir.z,
                       (child_origin.z + (lc.z + 1.0) * child_cell_size - entry_pos.z) * inv_dir.z, local_dir.z >= 0.0),
            );
        }
    }
    return result;
...
fn march_cartesian(
    ...
    loop {
        ...
        if tag == 1u {
            ...
            if kind == 3u {
                // Rotated45Y: dispatch rotated cartesian DDA in this cell.
                // The walker transforms the ray into the rotated local
                // [0, 3)³ frame and runs a standard cartesian DDA inside,
                // then maps the hit back to world. Subtree can be any
                // cartesian depth — rotation is applied once on entry.
                let rot_min = cur_node_origin + vec3<f32>(cell) * cur_cell_size;
                let rh = march_rotated45y_subtree(
                    child_idx, rot_min, cur_cell_size,
                    ray_origin, ray_dir,
                );
                if rh.hit {
                    return rh;
                }
                let m_rot = min_axis_mask(cur_side_dist);
                s_cell[depth] = pack_cell(cell + vec3<i32>(m_rot) * step);
                cur_side_dist += m_rot * delta_dist * cur_cell_size;
                normal = -vec3<f32>(step) * m_rot;
                continue;
            }
