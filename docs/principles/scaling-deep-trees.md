...
The world is a recursive base-3 tree. Every node has 27 children (3×3×3). The GPU ray marcher casts one ray per pixel, stepping through the tree via DDA at each level. Per-pixel LOD stops descent when a cell is sub-pixel. See `docs/architecture/rendering.md` for the full design.
